       ...
       public:
        ...
        void async_read_with_chat(ServerNetwork &network, ChatHandler chatHandler)
        {
            ...
            boost::asio::async_read(this->_socket,
                ...
                [this, header, &network, chatHandler](
                    ...
                    if (*header == ::network::PacketType::TCPInfo) {
                        ...
                        boost::asio::async_read(this->_socket,
                            boost::asio::buffer(data.get(), sizeof(ClientTCPReceivedInfo)),
                            [this, data, &network, chatHandler](
                                const boost::system::error_code &err, size_t) {
                                if (err) {
                                    utils::Logger::debug(
                                        std::format("Error in TCP read body: {}", err.message()));
                                    return;
                                }
                                network.notify(*data);
                    ...
                    if (*header == ::network::PacketType::ChatSend) {
                        auto chat = std::make_shared<::network::ClientSendMessage>();
                        boost::asio::async_read(this->_socket,
                            boost::asio::buffer(chat.get(), sizeof(::network::ClientSendMessage)),
                            [this, chat, &network, chatHandler](
                                const boost::system::error_code &err, size_t) {
                                if (err) {
                                    utils::Logger::debug(
                                        std::format("Error in TCP read chat: {}", err.message()));
                                    return;
                                }
